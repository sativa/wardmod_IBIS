...
c 
c see also humid.f for related functions
c ---------------------------------------------------------------------
c
...
c
     ...
     >           dsat6 =  7.1310977e-12 )
c
c constant used in calculations involving q (e.g., in qsat, dqsat,
c e_from_q)
      real qk
c
      parameter (qk = 0.622)
c 
c constants used in conversion between vapor pressure and dew point temperature
c from Campbell & Norman 1998 ("Introduction to Environmental Biophysics"), p. 41
c
      real tdew_a, tdew_b_w, tdew_c_w, tdew_b_i, tdew_c_i
c
      parameter (tdew_a = 611.,      ! n/m**2: same value regardless of the value of tdew
     >           tdew_b_w = 17.502,  ! unitless: for calculations with tdew >= 0 deg C (i.e., over water)
     >           tdew_c_w = 240.97,  ! deg C: for calculations with tdew >= 0 deg C (i.e., over water)
     >           tdew_b_i = 21.87,   ! unitless: for calculations with tdew < 0 deg C (i.e., over ice)
     >           tdew_c_i = 265.5)   ! deg C: for calculations with tdew < 0 deg C (i.e., over ice)
c
...
c and/or small p1
c
c WJS 02.14.10: This function is actually more generally applicable: it
c calculates any specific humidity from vapor pressure (i.e., it does
c not have to work with saturation values).
c
       qsat (e1, p1) = qk * e1 /
     >               max ( p1 - (1.0 - qk) * e1, qk * e1 )
c
c statement function dqsat is d(qsat)/dt, with t in deg k and q1
c in kg/kg (q1 is *saturation* specific humidity)
c
       dqsat (t, q1) = desat(t) * q1 * (1. + q1*(1./qk - 1.)) /
     >                 esat(t)
c
c statement function e_from_q calculates vapor pressure (n/m**2) from
c specific humidity (q1) and ambient pressure (p1: n/m**2); derived by
c inverting qsat function
c
       e_from_q (q1, p1) = q1*p1 / (qk + q1*(1.0 - qk))
c
