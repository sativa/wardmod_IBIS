...
c
       do 100 i = 1, npoi
         if(cmask(i) == 0) cycle  !skip the computation, continue to next point
c
c ---------------------------------------------------------------------
...
c in the model  - and uses totts = 1
c
c after that, litterfall is assumed zero for crops contributions
c and harvest date is reset
c
          do 80 j = scpft, ecpft
...
c
c 5/18/07 CJK : modify rates for the US based on Simon's dataset that adjusts
c the rates each year (from actual data - 1940-1999)
c
c            if (iyear .ge. 1940 .and. iyear .le. 1999) deposn(i) = deposn(i) *
c    >                           ndepfact(i,iyear+1-1940) 
c
c           if (iyear .gt. 1999) deposn(i) = deposn(i) *
c    >                           ndepfact(i,60) 
c
c CJK 5/18/07 to recycle for the years 1981-2000 after one cycle
c
c           if (iyear .gt. 1999) deposn(i) = deposn(i) *
c    >                           ndepfact(i,iyear+1-20-1940) 
c
