...
c
     ...
     >  w2(npoi,nsoilay)          !  '
c
      integer:: n_neighbors,idx_neighbors(8),i_neighbor,tmp_count,itmp
      save dmin, rimp
...
c
          ...
          if(flg_sandlayers == 1 .and. k == nsoilay) then !impose limiter to last but one layer
            wsoia(i,k) = min(wsoia(i,k),0.6)
            wsoib(i,k) = min(wsoib(i,k),0.6)
          end if
c
  120   continue
c
...
c
        ...
        wsoib(i,k) = min (wsoim(i,k), 1.0)
c
        if(flg_sandlayers == 1 ) then !impose limiter to last layer
          wsoia(i,k) = min(wsoia(i,k),owsoi(i,k-1),0.55)
        end if
...
!and then everything related to them.
      ...
      do i=1,npoi
         ...
         end do  !on k=1,nsoilay
      end do  !on i=1,npoi

      do i = 1, npoi
        if(cmask(i) == 0 ) cycle

        if(flg_sandlayers == 1 .and. wsoi(i,nsoilay) > 0.9) then !layer of sand is about to saturate, make correction for the whole soil column

          call find_neighbors(i,n_neighbors,idx_neighbors)

          tmp_count = 0 
          wsoi(i,:) = 0.0
          do itmp = 1, n_neighbors
            i_neighbor = idx_neighbors(itmp)
            if(wsoi(i_neighbor,nsoilay) < 0.9) then
              wsoi(i,:) = wsoi(i,:)+wsoi(i_neighbor,:)
              tmp_count = tmp_count + 1
            end if
          end do  !on itmp = 1, n_neighbors

          if(tmp_count > 0 ) then
            wsoi(i,:) = wsoi(i,:)/float(tmp_count)
          else  !all neighbors are problematic
            wsoi(i,:) = 0.5   !reset to default
          end if

        end if !on if flg_sandlayers and wsoi > 0.9
      end do !on i = 1, npoi
