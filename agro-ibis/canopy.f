...
c        slw(i) = min (fwetlx(i) * sl(i),
        slw(i) = min (fwetlx(i) * sl(i) * max(0.1, greenfracl(i)),
     >                0.8 * (wliql(i) + wsnol(i))
...
c
      do 400 i = 1, npoi
        if(cmask(i) == 0 ) cycle
c
!Y.Li add if statement to avoid potential NaN

        if(abs(vec(i,1))-273.16<60.0.and.abs(vec(i,2))-273.16<60.0.and.
     *     abs(vec(i,3))-273.16<60.0.and.abs(vec(i,4))-273.16<60.0.and.
     *     abs(vec(i,5))-273.16<60.0.and.abs(vec(i,8))-273.16<60.0.and.
     *     abs(vec(i,9))-273.16<60.0 ) then
...
!Y.Li - constraint wimp to [0,1],otherwise can cause unphysical solution
        if(abs(wimp(i))>1.0) then
          wimp(i) = 1.0
        end if
c
  100 continue
...
c        xai(i)   = 2.0 * lai(i,2) ! leaf area index does not need to be doubled (JCP 4/16/10)
        xai(i)   = lai(i,2)
        rain(i)  = rainu(i)
        train(i) = trainu(i)
        snow(i)  = snowu(i)
...
c        xai(i) = 2.0 * sai(i,2) ! stem area index does not need to be doubled (JCP 4/16/10)
        xai(i) = sai(i,2)
  200 continue
c
c step upper stems
...
c        xai(i)   = 2.0 * (lai(i,1) + sai(i,1)) ! LAI + SAI does not need to be doubled (JCP 4/16/10)
...
c
      do 100 i=1,npoi
        ...
        fveg(i) = fu(i)
c        xai(i) = 2.0 * lai(i,2)
        xai(i)  = lai(i,2) ! leaf area index does not need to be doubled (JCP 4/19/10)
  100 continue
c
c step upper leaves
c
...
c
      do 200 i=1,npoi
        ...
        fveg(i) = fu(i)
c        xai(i) = 2.0 * sai(i,2)
        xai(i)  = sai(i,2) ! stem area index does not need to be doubled (JCP 4/19/10)
  200 continue
c
c step upper stems
c
...
c
      do 400 i=1,npoi
        ...
        fveg(i) = (1.-fi(i))*fl(i)
c        xai(i) = 2.0 * (lai(i,1) + sai(i,1))
        xai(i)  = (lai(i,1) + sai(i,1)) ! LAI+SAI does not need to be doubled (JCP 4/19/10)
  400 continue
c
c step lower veg
c
